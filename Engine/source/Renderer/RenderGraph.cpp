...
namespace Renderer
{
	RenderGraph::RenderGraph(D3D12RHI& gfx)
	{
		for (UINT n = 0; n < gfx.GetTargetBuffers().size(); n++)
		{
			...
			RenderGraph::m_frameResourceHandles["Swap_Chain_" + std::to_string(n)] = gfx.LoadResource(std::shared_ptr<RenderTarget>(rt), ResourceType::Texture);
		}

		RenderGraph::m_frameResourceHandles["Depth_Stencil"] = gfx.LoadResource(std::make_shared<DepthStencil>(gfx), ResourceType::Texture);
	}
