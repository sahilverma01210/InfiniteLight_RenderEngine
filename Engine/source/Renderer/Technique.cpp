...
namespace Renderer
{
	Technique::Technique(size_t channel)
		:
		m_channel{ channel }
	{}

	Technique::Technique(std::string name, size_t channel, bool startActive) noexcept(!IS_DEBUG)
		:
	...
	{
		if (m_active && ((m_channel & channelFilter) != 0))
		{
