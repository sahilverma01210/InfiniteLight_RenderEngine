        ...
        for (int i = 0; i < PathTracingPassCB.bounceCount; ++i)
        {
            ...
            if (TraceRay(ray, info))
            {
                ...
                if (SampleLightRIS(rng, worldPosition, worldNormal, lightIndex, lightWeight))
                {
                    ...
                    pathRadiance += lightWeight * (directLighting + matProperties.emissive) * throughput / pdf;
                }
            ...
            else
            {
                ...
                pathRadiance += envMapTexture.SampleLevel(LinearWrapSampler, ray.Direction, 0).rgb * throughput / pdf;
                break;
            }
        }
