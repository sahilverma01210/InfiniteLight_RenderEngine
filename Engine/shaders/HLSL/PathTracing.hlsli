...
{
    uint M = min(RIS_CANDIDATES_LIGHTS, frameData.lightCount);
    lightIndex = -1;
    sampleWeight = 0.0f;
    ...
    for (int i = 0; i < M; ++i)
    {
        ...
        SampleSourceLight(frameData.lightCount, rng, lightIndex, sourcePdf);
        ...
        float targetPdf = Luminance(DoAttenuation(distance, light.range) * light.diffuseColor.rgb);
        if (light.type == DIRECTIONAL_LIGHT)
        {
            targetPdf = Luminance(light.diffuseColor.rgb);
        }
