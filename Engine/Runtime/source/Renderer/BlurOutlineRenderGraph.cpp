...
namespace Renderer
{
		...
		RenderGraph(gfx)
	{
		...
		//		blurKernel = std::make_shared<ConstantBuffer>(gfx, buf, 0);
		...
		//		Buffer buf{ std::move(l) };
		//		blurDirection = std::make_shared<ConstantBuffer>(gfx, buf, 1);
		//		AddGlobalSource(DirectBindableSource<ConstantBuffer>::Make("blurDirection", blurDirection));
		//	}
		//}
